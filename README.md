...
*   **后端框架**: Django 5.2.3 + Channels 4.3.2 (ASGI支持) 🐍
*   **数据库**: SQLite (默认, 支持配置为PostgreSQL/MySQL) 💾
...
*   **用户管理**: 用户注册、登录、登出、权限控制（普通用户、一般管理员、超级管理员）。管理员可管理用户状态、重置密码、删除用户，支持单个添加和批量添加用户，支持导出用户列表。 👥
*   **产品管理**: 产品的分类、详细信息（喉深、喉宽、变压器、加压力等）的录入、查询、展示。支持多种产品类型和参数配置，使用枚举类型定义标准可选项。 📦
*   **搜索功能**: 支持多条件组合搜索产品，包括分类、图号、子分类类型以及数值范围搜索（行程、加压力、重量、喉深、喉宽）。支持缓存优化搜索性能。 🔍
*   **文件管理**: 支持PDF、STEP、BMP等产品相关文件的下载、批量下载和在线预览。系统提供文件与产品信息的同步功能，支持查看未匹配文件列表。 📄
*   **异步压缩**: 支持异步压缩PDF、STEP、BMP等产品文件，提升大文件处理效率和用户体验。 📦⚡
*   **数据管理**: 支持通过CSV文件批量导入产品数据，以及导出产品、用户、日志、反馈数据为CSV格式。 📊
*   **系统日志**: 详细记录用户登录、登出、搜索、文件下载、数据导入导出、用户管理等操作，支持JSON格式日志输出，方便审计和问题追踪。 📋
*   **用户配置管理**: 为每个用户提供独立的配置，包括客户名称、密码有效期、文件下载限制以及下载统计等。自动为新用户创建配置，超级管理员默认密码永久有效。 ⚙️
*   **仕样链接管理**: 管理员可以创建和管理特定的搜索链接，包含预设的搜索配置，支持设置链接有效期、最大点击次数等。支持表单全选功能和AI搜索开关。 🔗
*   **下载数据分析**: 提供API接口分析下载数据，支持按时间范围和用户筛选，包含趋势数据、用户统计和总计数据。 📈
*   **AI智能搜索**: 集成了AI智能搜索功能，支持腾讯元器（默认）和Coze两种AI引擎，用户可在搜索界面切换。仕样链接可配置是否启用AI搜索。 🤖
*   **用户反馈功能**: 允许用户向管理员提交使用过程中的问题和建议，支持反馈分类、状态管理和用户通知，支持导出反馈数据，帮助持续改进系统。 💬
*   **产品文件水印**: 为下载的PDF文件自动添加水印，包含用户信息和时间戳，增强文件安全性。 🔒
*   **PDF在线预览**: 支持在浏览器中直接预览PDF文件，无需下载，提供更好的用户体验。 👁️
*   **用户个人中心**: 用户可查看和编辑个人资料，包括修改密码、查看下载统计、查看密码过期时间等。 🧑‍💼
*   **自动备份系统**: 定时自动备份数据库，支持配置备份周期和保留天数，自动清理过期备份和临时压缩文件。 💾
*   **安全防护**:
    *   速率限制中间件：防止恶意请求，支持全局和搜索特定的速率限制 🛡️
    *   安全响应头中间件：添加CSP、HSTS、X-XSS-Protection等安全响应头 🔐
    *   受限的Django Admin站点：仅超级管理员可访问后台管理 🚫
    *   受保护的媒体文件访问：需要登录认证才能下载文件 🔒
    *   HTTPS安全配置：支持通过环境变量配置SSL重定向、HSTS等 🔒
*   **环境配置**: 支持通过`.env`文件管理敏感配置（密钥、API令牌、数据库连接等），便于不同环境部署。 🔧
*   **API接口**: 提供多个API接口，包括下载分析、AI搜索、用户配置数据、Gitee Releases版本查询等。 🌐
...
*   **URL配置**: 定义URL路由，将URL映射到相应的视图函数，支持中英文双路由。 🗺️
*   **中间件**:
    *   `LoggingMiddleware`（clamps应用）：记录用户请求日志和处理时间 ⏱️
    *   `RateLimitMiddleware`（clamps应用）：速率限制控制，支持全局和搜索特定限制 🛡️
    *   `SecurityHeadersMiddleware`（项目级）：添加CSP、HSTS、X-XSS-Protection等安全响应头 🔐
    *   `GZipMiddleware`：响应压缩，提升传输效率 📦
*   **信号处理**: 通过 `signals.py` 实现用户创建时自动创建UserProfile，超级管理员默认密码永久有效。 📡
*   **静态文件**: 存放CSS、JS、图片、PDF文档等前端资源。 📁
*   **媒体文件**: 存放产品相关文件（STEP、PDF、BMP等），通过受保护视图访问。 📂
*   **PDF处理**: 通过 `pdf_utils.py` 实现PDF文件的水印添加和处理。 📄
*   **备份系统**: 通过 `backup.py` 实现定时数据库备份和临时文件清理。 💾
*   **缓存系统**: 使用Django LocMemCache缓存优化搜索性能。 ⚡
*   **环境配置**: 支持通过 `.env` 文件和 `python-dotenv` 管理敏感配置。 🔧
...
│   ├── cfg/                    # 配置文件
│   │   ├── local/              # 本地环境启动脚本
│   │   │   ├── start_Gaoj-server.bat         # Windows启动脚本
│   │   │   ├── start_Gaoj-server-NoWindow.bat # 无窗口启动脚本
│   │   │   └── start_Gaoj-server-NoWindow.pyw # 无窗口Python启动脚本
│   │   ├── server/             # 服务器环境启动脚本
│   │   │   ├── start_Tserver.bat              # 服务器启动脚本
│   │   │   ├── start_Tserver-NoWindow.bat     # 无窗口服务器启动脚本
│   │   │   ├── start_Tserver-NoWindow.pyw     # 无窗口Python启动脚本
│   │   │   ├── start_Tserver.py               # Python服务器启动脚本
│   │   │   └── backup_Tdb.bat                 # 数据库备份批处理
│   │   ├── pip.conf            # pip配置文件（Linux/macOS）
│   │   └── pip.ini             # pip配置文件（Windows）
│   ├── code_review/            # 代码审查工具
│   │   ├── add_copyright.py    # 添加版权头工具
│   │   └── check_copyright.py  # 检查版权头工具
│   ├── csv/                    # CSV数据文件
│   │   ├── X2C-C.csv           # X2C-C分类数据
│   │   ├── X2C-V2-C.csv        # X2C-V2-C分类数据
│   │   ├── X2C-V2-X.csv        # X2C-V2-X分类数据
│   │   ├── X2C-V3-C.csv        # X2C-V3-C分类数据
│   │   ├── X2C-V3-X.csv        # X2C-V3-X分类数据
│   │   ├── X2C-X.csv           # X2C-X分类数据
│   │   └── process_csv.py      # CSV处理工具
│   ├── data/                   # 数据文件
│   │   ├── Batch_add_UTF8.csv  # 批量添加用户模板
│   │   └── 产品数据表头.xls     # 产品数据表头参考
│   ├── deployment_scripts/     # 部署脚本
│   │   ├── DEPLOYMENT.md       # 部署说明文档
│   │   ├── install_service.py  # Windows服务安装脚本
│   │   ├── start_production.bat # 生产环境启动脚本
│   │   └── start_wsgi.py       # WSGI服务器启动脚本
│   ├── doc/                    # 文档
│   │   ├── csv文件导入前数据标准化流程.md
│   │   ├── 产品搜索使用指南.md
│   │   ├── Product Search User Guide.md
│   │   ├── 小原焊枪选型数据库项目源码商用分析报告.md
│   │   ├── 小原焊钳选型数据库项目部署标准化流程.md
│   │   ├── 获取media文件标准化流程.md
│   │   ├── 隐私政策.md
│   │   ├── Privacy Policy.md
│   │   ├── 用户协议.md
...
│   │   ├── rename_pdf_files.py # PDF文件重命名工具
│   │   └── delete_r_ending_files.py # 删除R结尾文件工具
│   ├── howtogetstep/           # STEP文件处理工具
│   │   ├── compare_files.py    # 文件比较工具
│   │   ├── comparison_result.csv # 比较结果
│   │   └── copy_step_by_status.py # 按状态复制STEP文件
...
│   ├── pdf_viewer.html         # 中文版PDF预览页面
│   ├── pdf_viewer_en.html       # 英文版PDF预览页面
...
│   ├── __init__.py
│   ├── asgi.py                 # ASGI 配置文件
│   ├── middleware.py           # 项目级中间件（SecurityHeadersMiddleware）
│   ├── settings.py             # 项目设置文件
│   ├── urls.py                 # 项目主 URL 配置
│   └── wsgi.py                 # WSGI 配置文件
...
*   **查看用户列表**: 在用户管理页面，管理员可以查看所有注册用户的列表，包括用户名、邮箱、是否活跃、是否为超级用户、注册日期、最后登录时间和创建者信息。
*   **添加单个用户**: 管理员可以添加单个用户，设置用户名、邮箱、密码，并可选择是否为管理员。系统会自动创建用户的UserProfile配置。此操作会记录日志。
*   **批量添加用户**: 管理员可以通过上传CSV文件批量添加用户，支持下载CSV模板。此操作会记录日志。
...
*   **导出用户列表**: 管理员可以将用户列表导出为CSV文件，方便进行离线分析或备份。
...
5.  点击"添加用户"按钮添加单个用户，或点击"批量添加"按钮上传CSV文件批量添加用户。
6.  点击"导出用户"按钮将用户列表导出为CSV文件。
...
系统提供PDF文件在线预览功能，用户无需下载即可直接在浏览器中查看PDF文档。

#### ✨ 4.5.1 核心功能

*   **无需下载**: 用户可以直接在浏览器中预览PDF文件，节省本地存储空间。
*   **多页导航**: 支持翻页、跳转到指定页面等导航功能。
*   **缩放控制**: 支持放大、缩小和自适应页面大小。
*   **双语支持**: 提供中英文双语界面。
*   **权限控制**: 预览功能同样受到用户权限控制，未授权用户无法预览受保护文件。

#### 📥 4.5.2 操作步骤

1.  在产品详情页面，找到PDF文件预览区域。
2.  点击"预览"按钮打开PDF预览页面。
3.  在预览页面中可以翻页、缩放查看文档内容。
4.  如需下载，可点击下载按钮保存文件。
...
系统为每个用户提供个人中心页面，用户可以查看和管理个人信息。

*   **个人资料查看**: 用户可以查看自己的用户名、邮箱、注册日期、最后登录时间等信息。
*   **密码管理**: 用户可以在个人中心修改自己的密码。
*   **配置信息查看**: 用户可以查看自己的密码过期时间、下载统计、客户名称等配置信息。
*   **下载统计**: 用户可以查看自己的文件下载次数和下载大小统计。
...
*   **反馈提交**: 用户可以通过系统界面的反馈入口提交反馈信息，包括问题描述、相关链接和联系方式。
*   **反馈管理**: 管理员可以在系统后台查看和处理用户提交的反馈，包括更新状态、标记已通知用户等。
*   **反馈导出**: 管理员可以将用户反馈数据导出为CSV文件，方便进行离线分析或统计。
*   **反馈日志**: 系统会记录所有反馈提交和处理的日志，便于追踪和分析用户反馈趋势。

**操作步骤**：

1.  用户登录系统后，点击页面上的反馈按钮或链接（`/feedback/` 或 `/feedback_en/`）。
2.  在反馈表单中填写反馈类型、详细描述和联系方式。
3.  点击“提交反馈”按钮。
4.  管理员登录系统后台，导航到"管理仪表盘" -> "用户反馈"（`/management/user_feedback/`）。
5.  对反馈进行处理，更新状态或标记已通知用户。
6.  点击"导出反馈"按钮将反馈数据导出为CSV文件。
...
*   **AI搜索接口**: 提供 `ai_search_api` 接口，支持POST请求调用。
*   **Coze集成**: 通过 `coze_service.py` 模块与Coze API进行交互。
*   **智能搜索**: 支持自然语言查询，返回相关产品信息。
*   **仕样链接AI搜索**: 仕样链接可配置是否启用AI搜索功能。
...
| 接口 | URL | 说明 |
|------|-----|------|
| 下载分析API | `/management/api/download-analysis/` | 分析下载数据，支持按时间范围和用户筛选 |
| AI搜索API | `/ai_search_api/` | AI智能搜索接口 |
| 用户配置数据API | `/api/user_profile_data/` | 获取当前用户的配置数据 |
| Gitee Releases API | `/api/gitee/releases/latest/<owner>/<repo>/` | 查询Gitee仓库最新Release版本 |
...
系统内置了自动备份功能，确保数据的安全性和可靠性，防止数据丢失。

*   **自动备份**: 系统会定期自动备份数据库，默认备份周期为每日一次。
*   **备份存储**: 备份文件将存储在 `backup` 目录中，支持配置备份保留天数（默认3天）。
*   **备份清理**: 自动清理超过保留天数的过期备份文件。
*   **临时文件清理**: 自动清理 `tmp` 目录下超过24小时的临时压缩文件。

**配置与使用**：

1.  备份配置在 `backup.py` 中，可通过修改 `BACKUP_DIR` 和 `KEEP_DAYS` 参数调整备份目录和保留天数。
2.  备份任务通过 `schedule` 库定时执行，可在项目启动时自动启动。
3.  如需手动备份，可运行备份脚本或通过管理界面触发。
...
*   **安全响应头**: 通过 `SecurityHeadersMiddleware` 中间件自动添加以下安全响应头：
    ...
    *   X-XSS-Protection
    *   X-Frame-Options
*   **速率限制**: 通过 `RateLimitMiddleware` 中间件实现请求速率限制，防止恶意请求。
*   **受限的Admin站点**: 通过 `RestrictedAdminSite` 限制仅超级管理员可访问Django Admin后台。
*   **受保护的媒体文件**: 通过 `protected_media` 视图实现需要登录认证才能下载文件。
*   **HTTPS支持**: 支持通过环境变量配置SSL重定向、HSTS等HTTPS安全设置。
*   **环境变量管理**: 敏感配置（密钥、API令牌、数据库连接等）通过 `.env` 文件管理，避免硬编码。
...
# Gitee API令牌
GITEE_API_TOKEN=your-gitee-api-token
```
