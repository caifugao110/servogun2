...
*   **部署环境**: Python 3.11+ ⚡
*   **核心库**: PyPDF2 (PDF处理), chardet (编码检测), reportlab (PDF水印生成), pdf.js (PDF预览)

### ✨ 2.2 核心功能模块

*   **用户管理**: 用户注册、登录、登出、权限控制（普通用户、一般管理员、超级管理员）。管理员可管理用户状态、重置密码、删除用户，支持批量添加用户。 👥
*   **产品管理**: 产品的分类、详细信息（喉深、喉宽、变压器、加压力等）的录入、查询、展示。支持多种产品类型和参数配置。 📦
*   **搜索功能**: 支持多条件组合搜索产品，包括分类、图号、子分类类型以及数值范围搜索（行程、加压力、重量、喉深、喉宽）。 🔍
*   **文件管理**: 支持PDF、STEP等产品相关文件的下载、批量下载和在线预览。系统提供文件与产品信息的同步功能。 📄
...
*   **API接口**: 提供多个API接口，包括下载分析、AI搜索、用户配置数据、Gitee Releases等。 🌐
...
*   **中间件**: 例如 `LoggingMiddleware` 用于记录用户请求日志，`RateLimitMiddleware` 用于限流控制。 🛡️
